...
为便于检索，本文档中的审稿意见已依照一篇规范的 SCIE 期刊论文的呈现位置进行升序排序，呈现位置相同时，依照重要性进行降序排序。
...
Figures containing subplots should follow the following guidelines. Initially, for each subplot, there should be a caption containing a marker (like (a), (b), and (c)) and a description. The caption of each subplot should be editable outside the subplot instead of being embedded in the subplot. 
